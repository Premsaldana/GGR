...
type AsyncTransaction = Pick<typeof db, 'select' | 'insert'>;
const postgresDb = db as unknown as {
  transaction<T>(callback: (tx: AsyncTransaction) => Promise<T>): Promise<T>;
};

async function ensureAllowedUnitsPostgres() {
  const existingUnits = await db.select().from(units).execute();
  const existingByType = new Map(existingUnits.map((unit) => [getCanonicalRoomType(unit), unit] as const).filter((entry) => entry[0] !== null) as Array<readonly [string, typeof existingUnits[number]]>);
  for (const allowedUnit of ALLOWED_UNITS) {
    ...
    if (existing) {
      if (!existing.active || existing.displayName !== allowedUnit.displayName) {
        await db.update(units).set({ displayName: allowedUnit.displayName, active: true, capacityAdults: allowedUnit.capacityAdults, capacityChildren: allowedUnit.capacityChildren, defaultCheckInTime: DEFAULT_CHECK_IN_TIME, defaultCheckOutTime: DEFAULT_CHECK_OUT_TIME }).where(eq(units.id, existing.id)).execute();
      }
      continue;
    }
    await db.insert(units).values({ id: crypto.randomUUID(), displayName: allowedUnit.displayName, slug: allowedUnit.slug, active: true, capacityAdults: allowedUnit.capacityAdults, capacityChildren: allowedUnit.capacityChildren, defaultCheckInTime: DEFAULT_CHECK_IN_TIME, defaultCheckOutTime: DEFAULT_CHECK_OUT_TIME }).execute();
  }
}
...
export async function getUnits() {
  await requireAdmin();
  const existingUnits = databaseProvider === 'postgres'
    ? await ensureAllowedUnitsPostgres().then(() => db.select().from(units).where(eq(units.active, true)).execute())
    : (ensureAllowedUnits(), db.select().from(units).where(eq(units.active, true)).all());
...
export async function getReservations(monthStart: string, monthEnd: string) {
  await requireAdmin();
  const query = db.select().from(reservations).where(
    and(
  ...
  return databaseProvider === 'postgres' ? await query.execute() : query.all();
}
  ...
  try {
    ...
    result = await postgresDb.transaction(async (tx) => {
      ...
      if (generatedLineItems.length > 0) {
        ...
        await tx.insert(reservationLineItems).values(items).execute();
      }
...
export async function issueInvoiceAction(reservationId: string, clientDepositMinorUnits?: number) {
  try {
    ...
    const result = databaseProvider === 'postgres'
      ? (await dbReady, await postgresDb.transaction(async (tx) => {
      // 1. Reload the reservation and line items from the database
      const reservation = (await tx.select().from(reservations).where(eq(reservations.id, reservationId)).execute())[0];
      if (!reservation) {
      ...
      const guest = (await tx.select().from(guests).where(eq(guests.id, reservation.guestId)).execute())[0];
      const unit = (await tx.select().from(units).where(eq(units.id, reservation.unitId)).execute())[0];
