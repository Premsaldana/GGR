...
export async function verifyProofAction(proofId: string, verifiedAmountMinorUnits: number, paymentMode: string, paymentReference: string, adminNote: string) {
  try {
    ...
    const verifiedProofQuery = db.select().from(paymentProofs).where(eq(paymentProofs.id, proofId));
    const verifiedProof = provider === 'postgres' ? (await dbReady, (await verifiedProofQuery.execute())[0]) : verifiedProofQuery.get();
    const verifiedInvoiceQuery = verifiedProof ? db.select().from(invoices).where(eq(invoices.id, verifiedProof.invoiceId)) : null;
    const verifiedInvoice = verifiedInvoiceQuery ? (provider === 'postgres' ? (await dbReady, (await verifiedInvoiceQuery.execute())[0]) : verifiedInvoiceQuery.get()) : null;
...
export async function rejectProofAction(proofId: string, adminNote: string, requestResubmit: boolean = false) {
  try {
    ...
    const rejectedProofQuery = db.select().from(paymentProofs).where(eq(paymentProofs.id, proofId));
    const rejectedProof = provider === 'postgres' ? (await dbReady, (await rejectedProofQuery.execute())[0]) : rejectedProofQuery.get();
    const rejectedInvoiceQuery = rejectedProof ? db.select().from(invoices).where(eq(invoices.id, rejectedProof.invoiceId)) : null;
    const rejectedInvoice = rejectedInvoiceQuery ? (provider === 'postgres' ? (await dbReady, (await rejectedInvoiceQuery.execute())[0]) : rejectedInvoiceQuery.get()) : null;
...
export async function finalizeInvoiceAction(reservationId: string) {
  try {
    ...
    if (provider === 'postgres') {
      ...
      const existingInvoices = await tx.select().from(invoices).where(eq(invoices.reservationId, reservationId)).orderBy(desc(invoices.version)).execute();
    ...
    const finalizedInvoiceQuery = db.select().from(invoices).where(eq(invoices.id, result.invoiceId));
    const finalizedInvoice = provider === 'postgres' ? (await dbReady, (await finalizedInvoiceQuery.execute())[0]) : finalizedInvoiceQuery.get();
