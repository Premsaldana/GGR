...
export default async function InvoicesPage() {
  const rowsQuery = db.select({ invoice: invoices, reservation: reservations, guest: guests, unit: units })
    .from(invoices).innerJoin(reservations, eq(invoices.reservationId, reservations.id))
    .innerJoin(guests, eq(reservations.guestId, guests.id)).innerJoin(units, eq(reservations.unitId, units.id))
    .orderBy(desc(invoices.createdAt));
  const rows = databaseProvider === 'postgres' ? (await dbReady, await rowsQuery.execute()) : rowsQuery.all();
